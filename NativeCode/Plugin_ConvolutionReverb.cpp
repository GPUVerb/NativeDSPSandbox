...
namespace ConvolutionReverb
{
    ...
    enum Param
    {
        ...
        P_USESAMPLE,
        P_REVERSE,
        P_NUM // P_NUM to fetch # of enums, not a parameter.
    }; 
    ...
    // forward declared by DECLARE_EFFECT (PluginList.h)
    ...
    // called when this DSP unit is created and can be null
    ...
    UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
    {
        ...
        for (int i = 0; i < inchannels; i++)
        {
            ...
            for (int n = 0; n < data->hopsize; n++)
            {
                ...
                // here's the lerp
                // std::lerp(input, gain * y[n].re, wet);
    ...
    int UNITY_AUDIODSP_CALLBACK GetFloatBufferCallback(UnityAudioEffectState* state, const char* name, float* buffer, int numsamples)
    {
        ...
        // not sure if a ui is necessary for gpuverb.
